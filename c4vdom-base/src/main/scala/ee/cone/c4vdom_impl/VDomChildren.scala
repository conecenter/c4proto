...
@c4component case class ChildPairFactoryImpl(createMapValue: MapVDomValueFactory) extends ChildPairFactory {
  def apply[C](
    key: VDomKey,
    theElement: VDomValue,
    elements: ViewRes
  ): ChildPair[C] = ChildPairImpl[C](key, createMapValue.create(
