...
lazy val `c4actor-base-examples` = project.settings(publishSettings).dependsOn(`c4actor-base`,`c4proto-types`, `c4gate-log4j`)
...
lazy val `c4actor-extra-examples` = project.settings(publishSettings).dependsOn(`c4actor-base`,`c4proto-types`, `c4gate-log4j`, `c4actor-extra`, `c4external-base`, `c4actor-kafka`)
lazy val `c4actor-kafka` = project.settings(publishSettings).dependsOn(`c4actor-base`)
lazy val `c4gate-server` = project.settings(publishSettings).dependsOn(`c4actor-kafka`, `c4gate-client`, `c4gate-log4j`, `c4gate-publish`)
lazy val `c4gate-consumer-example` = project.settings(publishSettings).dependsOn(`c4actor-kafka`, `c4gate-client`, `c4gate-log4j`)
...
lazy val `c4gate-sse-example` = project.settings(publishSettings).dependsOn(`c4proto-api`, `c4actor-kafka`, `c4ui-main`, `c4gate-publish`, `c4gate-client`, `c4vdom-canvas`, `c4gate-log4j`, `c4gate-repl`)
