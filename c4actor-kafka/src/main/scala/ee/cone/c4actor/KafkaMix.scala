...
trait KafkaConfigApp {
  ...
  private lazy val keyStorePassPath: String = config.get("C4AUTH_KEY_FILE")
  lazy val kafkaConfig: KafkaConfig = KafkaConfig(bootstrapServers,inboxTopicPrefix,maxRequestSize,keyStorePath,keyStorePassPath)()
}
