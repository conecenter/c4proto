...
trait S3Manager {
  def get(resource: String): Option[Array[Byte]]
  def put(resource: String, body: Array[Byte]): Unit
  def delete(resource: String): Boolean
}

@c4("S3ManagerApp") final class S3ManagerImpl(config: Config)(
  ...
  def sendInner(resource: String, date: String, authorization: String, builder: HttpRequest.Builder): Option[Array[Byte]] = {
    ...
    val resp = client.send(req,bodyHandler)
  ...
  def put(resource: String, body: Array[Byte]): Unit = {
    ...
    assert(send(resource, "PUT", contentType, builder).nonEmpty)
  }

  def delete(resource: String): Boolean =
    send(resource, "DELETE", "", HttpRequest.newBuilder().DELETE()).nonEmpty
