...
@c4("AkkaGatewayApp") final class WebSocketUtilImpl(akkaMat: AkkaMat) extends WebSocketUtil {
  ...
  def toResponse(up: UpgradeToWebSocket, flow: Flow[ByteString, ByteString, NotUsed])(implicit ec: ExecutionContext): HttpResponse =
    up.handleMessages(
      Flow[Message].mapAsync(1)(getData(_))
        .idleTimeout(5.seconds).via(flow).keepAlive(1.seconds,()=>ByteString.empty)
        .map(BinaryMessage(_))
    )
}
