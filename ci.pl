...
my $handle_build = sub{
    ...
    my @commands = (
        ...
        ["-t","docker","build","-t","${builder_reg}builder:$tag","-f","$ctx_dir/build.$mode.dockerfile",@args,$ctx_dir],
        $builder_reg ? ["docker","push","${builder_reg}builder:$tag"] : (),
        ["rm","-r",$ctx_dir],
        ["docker","create","--name",$builder,"${builder_reg}builder:$tag"],
