...
object ViewBuilderGenerator extends Generator {
  def get: Get = {
    case (code@q"@protocol(...$exprss) object ${objectNameNode@Term.Name(objectName)} extends ..$ext { ..$stats }", fileName) ⇒ Util.unBase(objectName, objectNameNode.pos.end) { objectName ⇒
      val origs: List[OrigInfo] = stats.collect {
        case q"..$mods case class ${Type.Name(messageName)} ( ..$params ) extends ..$ext"
          ...
          val fields = params.map {
            ...
            case t: Tree ⇒
              Utils.parseError(t, "Getters", fileName)
          }
