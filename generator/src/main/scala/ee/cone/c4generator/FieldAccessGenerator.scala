...
object FieldAccessGenerator extends Generator {
  def get: Get = { case (Defn.Object(Seq(mod"@fieldAccess"),baseObjectNameNode@Term.Name(baseObjectName),code), fileName) ⇒ Util.unBase(baseObjectName,baseObjectNameNode.pos.end){objectName⇒
    ...
    val nCode = code.transform{
      case q"$o.of(...$args)" ⇒
        ...
        q"$o.ofSet(...$nArgs)"
      case t: Tree ⇒ Utils.parseError(t, "fieldAccess", fileName)
    }
