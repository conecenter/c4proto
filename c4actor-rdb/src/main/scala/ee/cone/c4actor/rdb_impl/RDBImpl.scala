...
case class FromExternalDBSyncTransform(srcId:SrcId) extends TxTransform {
  def transform(local: World): World = WithJDBCKey.of(local){ conn ⇒
    ...
    if(messages.isEmpty) local else {
      ...
      val parser = new IndentedParser(' ',"\n",StringToUniversalPropImpl.converters.toMap)
      val universalNode = parser.parse(textEncoded)
...
class DDLGeneratorImpl(
  ...
  private def esc(handler: String, expr: String) =
    bodyStatements(s"return chr(10) || key || ' ' || '$handler' || replace($expr, chr(10), chr(10)||' ');" :: Nil)
  private def getId(i: Long) = "'0x%04x'".format(i)
  private def longStrType = toDbName("String","t",1)
  private def isSysType(name: String) =
    name == toDbName("String","t").toLowerCase ||
      name == toDbName("BigDecimal","t").toLowerCase
...
class IndentedParser(
  ...
  def parse(data: String): UniversalNode = {
    val lines = data.split(lineSplitter).filter(_.nonEmpty).toList
