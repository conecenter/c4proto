...
trait HashSearchRangersApp{
  def hashSearchRangers: List[Ranger[_ <: Product, _]] = Nil
}
...
trait HashSearchRangerRegistryApi {
  def getByCl[By <: Product, Field](byCl: Class[By], fieldCl: Class[Field]): Option[Ranger[By, Field]]
  def getByName[By <: Product, Field](rangerName: String): Option[Ranger[By, Field]]
}

case class HashSearchRangerRegistryImpl(rangers: List[Ranger[_ <: Product, _]]) extends HashSearchRangerRegistryApi {
