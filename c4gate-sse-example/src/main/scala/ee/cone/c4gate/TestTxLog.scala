...
case class TestTxLogView(locationHash: String = "txlog")(
  ...
  def view: Context ⇒ ViewRes = untilPolicy.wrap { local ⇒
    ...
    } yield div(s"tx${updatesSummary.srcId}",List())(List(
      text("text",
...
case class UpdatesSummary(srcId: SrcId, objCount: Long, byteCount: Long, valueTypeIds: List[Long])
...
@assemble class TestTxLogAssemble(actorName: String)(
  ...
  def sumMeta(
    ...
    val skipIds = Seq(classOf[ToAlienWrite],classOf[HttpPublication])
